...
class HomeScreen extends ConsumerWidget {
  ...
  static const categories = ['All', 'Sports', 'Cruiser', 'Adventure', 'Scooters'];
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text(AppStrings.homeHeadline,
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ChoiceChip(
                    label: Text(label),
                    selected: selected,
                    onSelected: (_) => ref.read(categoryFilterProvider.notifier).state = label,
            ...
            const SizedBox(height: 18),
            const Text('Featured bikes',
                style: TextStyle(color: AppColors.textHigh, fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 14),
            Expanded(
              child: bikes.isEmpty
                  ? const Center(
                      child: Text('No bikes match your search yet.', style: TextStyle(color: AppColors.textMedium)),
                    )
                  : ListView.separated(
            ...
            const Text('Trending modifications',
                style: TextStyle(color: AppColors.textHigh, fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 14),
            accessoriesAsync.when(
              data: (items) => SizedBox(
                ...
                child: ListView.separated(
                  ...
                  separatorBuilder: (_, _) => const SizedBox(width: 12),
                  itemBuilder: (context, index) => _TrendingAccessoryCard(item: items[index]),
                ),
              ),
              loading: () => const SizedBox(height: 110, child: Center(child: CircularProgressIndicator())),
              error: (_, _) => const SizedBox(height: 110, child: Center(child: Text('Failed to load accessories', style: TextStyle(color: AppColors.error)))),
            ),
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(item.name, style: const TextStyle(color: AppColors.textHigh, fontWeight: FontWeight.bold, fontSize: 15)),
          const SizedBox(height: 8),
          Text(item.type, style: const TextStyle(color: AppColors.textMedium, fontSize: 13)),
          const Spacer(),
          Text('₹${item.price.toStringAsFixed(0)}', style: const TextStyle(color: AppColors.accent, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
